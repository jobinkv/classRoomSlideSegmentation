...
class PENNet(nn.Module):
    def __init__(self, num_classes, res101_path,pretrained=True, use_aux=True):
        super(PENNet, self).__init__()
        ...
        self.pen1 = nn.Sequential(
            nn.Conv2d(4096, 4096, kernel_size=1, padding=1, bias=False),
            nn.BatchNorm2d(512, momentum=.95),
            nn.ReLU(inplace=True),
            nn.Dropout(0.9)
        )
        self.pen2 = nn.Sequential(
            nn.Conv2d(4096, 4096, kernel_size=1, padding=1, bias=False),
            nn.BatchNorm2d(512, momentum=.95),
            nn.ReLU(inplace=True),
            nn.Dropout(0.9),
	    ...
	    #return F.adaptive_avg_pool2d(x, (1, 1)),F.adaptive_avg_pool2d(aux, (1, 1))
